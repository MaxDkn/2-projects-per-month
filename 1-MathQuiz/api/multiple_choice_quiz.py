...
My Personal To Do List:
  Currently
  - Algbra, (q_calculate_image): rework on the suggestions answers because the function is really complicated
...
from math import ceil, log10, sin, cos, radians, prod, gcd, lcm, floor
...
class Algebra(QuestionsMCQ):
    ...
    def q_calculate_antecedent(self, shuffle_the_equation: bool = True, *, a_interval: tuple = (-4, 4),
        ...
        # (c+b is k*a with k and a integers)
    ...
    def q_calcul_product(multiplication_tables_interval: tuple = (6, 12),
                         odds_for_11: float | None = 1 / 6) -> dict:
        """
...
class Arithmetic(QuestionsMCQ):
    ...
    def all_prime_number_of_an_interval(interval: tuple = (5, 50)) -> list[int]:
        ...
        """
        #  Check if the interval arg is just an tuple (or list) of two elements, sorted.
...
class Geometry(QuestionsMCQ):
    ...
    def convert_value_unit(self, source_value: int | float, source_prefix: str, target_prefix: str) -> float:
        """
...
def run(number_of_questions: int | None = None, subjects: list[str] | str = '*'):
    marge = 3*" "
    score = 0
    letters = 'ABCD'
