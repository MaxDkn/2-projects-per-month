...
want to use translation. BUT the questions and answers are in French because user that we'll play to my quiz
are French.
I split my programs in different files, one for each subject
in the subject repertory (./subject/). Maybe I will compile in only one file, but it's less readable...

My Personal To Do List:
  Currently
  - Geometry, (q_volume) : determiner before "pyramide" have to be "une", not "un"... This problem is only one the third
  sentences, with feminine word (so only "pyramide" shape) and this is really complicated to fix that, so I will
  probably not solve this "bug".
  ...
  - create unitest
  - French and English version (translate question) - currently it's only French mode.
